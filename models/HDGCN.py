...
class AHA(nn.Module):
    def __init__(self, in_channels, num_layers, CoM):
        ...
        groups = get_groups(dataset='FineDiving', CoM=CoM)
        ...
        self.edge_conv = EdgeConv(inter_channels, inter_channels, k=1)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=2,
        ...
        self.dataset = 'NTU' if num_point == 25 else 'FineDiving'
        ...
        self.l5 = TCN_GCN_unit(base_channels, base_channels*2, A, adaptive=adaptive, CoM=CoM)
    ...
    def forward(self, x):
        ...
        # N*M,C,T,V
        c_new = x.size(1)
        x_now = x.view(N, c_new, T, V).mean(3)
        x_number = x.view(N, M, c_new, -1).mean(3).mean(1)
        x_now = self.drop_out(x_now)

        return self.fc(x_number),x_now
